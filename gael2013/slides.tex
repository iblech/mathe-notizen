...
\frame[t]{\frametitle{Sheaves of modules, internally}
  ...
  \begin{itemize}
    \item $\F$ is locally of finite type iff internally, it is finitely
    generated:
    \begin{multline*}
    ...
    \item $\F$ is locally of finite presentation iff internally,
    it is finitely presented:
    \[ \Sh(X) \models \speak{$\exists$ surjection~$\O_X^n
    \twoheadrightarrow \F$ with f.g. kernel} \]

    \item $\F$ is locally free iff internally, it is a free module:
