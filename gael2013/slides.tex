...
\newcommand{\pointthis}[2]{
        ...
        callout relative pointer={(0.2cm,0.3cm)},fill=blue!20] at ($(#1.north)+(-.5cm,-0.9cm)$) {#2};}%
        }%
...
\frame[t]{\frametitle{Sheaves of modules, internally}
  ...
  \begin{itemize}
    ...
    \item $\F$ is \emph{coherent} iff internally, it is\ldots
  \end{itemize}
}
...
\frame[t]{\frametitle{Rank of a sheaf of modules}
  ...
  \begin{columns}[t,onlytextwidth]
    ...
    \begin{column}{0.5\textwidth}
      \begin{block}{Classical definition}
        \ \\[-0.5em]
        The \emph{rank function} of~$\F$ is
        \[ \begin{array}{@{}rcl@{}}
          X &\longrightarrow& \NN \\
  ...
  \begin{columns}[t,onlytextwidth]
    ...
    \begin{column}{0.5\textwidth}
      \begin{block}{Constructive linear algebra}
        \ \\[-0.5em]
        Let~$M$ be a f.\,g. module over a residue field. Then:
        \begin{itemize}
...
\frame[t]{\frametitle{The gros Zariski topos}
  \begin{definition}The \emph{gros Zariski topos}~$\Zar(S)$ of a scheme~$S$ is the
  category of sheaves~$(\Sch/S)^\mathrm{op} \to \Set$ defined on the Grothendieck
  situs~$\Sch/S$ of schemes over~$S$.\end{definition}
  ...
  \begin{itemize}
    ...
    \item Internally, $\ul{\AA^1_S}$ (affine line), given by
    \[ \ul{\AA^1_S} : X \longmapsto \Gamma(X, \O_X), \]
    looks like a field: $\Zar(S) \models \forall x\?\ul{\AA^1_S}\_ x \neq 0
...
\frame[t]{\frametitle{Conclusion}
  \begin{center}
    \hil{\large Ordinary mathematics is\ldots}
  \end{center}
  ...
  \begin{columns}[onlytextwidth]
    \begin{column}{0.5\textwidth}
      a technical device for simplifying boring proofs
    \end{column}
    vs.
    \hspace{0.5em}
    \begin{column}{0.5\textwidth}
