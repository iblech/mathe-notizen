...
\note{
  \begin{itemize}
    \item \begin{justify}Here is a constructive proof the first cited proposition in
    linear algebra:
    ...
    \item \begin{justify}In constructive mathematics, one can not show that any
    finitely generated vector space over a field admits a finite basis. This is not because
    the space might strangely turn out to be infinite-dimensional, but merely
    because one may not be able to explicitly exhibit a basis.\end{justify}
...
\frame[t]{\frametitle{The gros Zariski topos}
  ...
  \note{
    \begin{itemize}
      ...
      logical form holds for~$\ul{\AA}^1_{\Spec\ZZ}$ iff it holds for any local
      ring (in any universe, particularly~$\Set$).

      Therefore, in proving a statement of such a form about arbitrary local
      rings, one may assume that they even are fields.
...
\frame[t]{\frametitle{Open problems}
  ...
  \begin{itemize}
    ...
    \item Find interesting and useful properties of the internal language of
    specific schemes, for instance projective~$n$-space.
