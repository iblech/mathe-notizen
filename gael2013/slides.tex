...
\frame[t]{\frametitle{What is the internal language?}
  ...
  \note{
    \begin{itemize}
      ...
      see~\cite{johnstone:elephant}~(chapters~D1 and~D4)
      and~\cite{moerdijk:maclane:sheaves} (chapter~VI).
    \end{itemize}
  }
}
...
\note{
  \begin{itemize}
    ...
    of~\cite{johnstone:topos}):
    Then one can do category
    theory internal to~$\Sh(X)$ using the internal language.
...
\note{
  \begin{enumerate}
    ...
    negation -- recall the previous false rumor.] But one can show (quite
...
\note{
  \begin{itemize}
    \item Under the correspondence, locally constant upper-semicontinuous
    functions map exactly to the ordinary internal natural numbers, i.\,e. the
    global sections of~$N$.

    \item Here is an explicit example of a completed natural number: Let~$X =
    ...
    \begin{multline*}
      \min \{ n \in \NN \,|\, \speak{$\F$ can be generated by~$n$ elements} \} = \\
...
\note{
  \begin{itemize}
    ...
    would also generate~$M$. By minimality of~$m$, this is not possible. So
...
\note{
  ...
  \begin{itemize}
    ...
    \item Simplification rules making the Kripke-Joyal semantics more fun to use
